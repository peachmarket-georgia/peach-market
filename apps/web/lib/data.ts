/**
 * 더미 데이터 - PRD 기준 USD / 영문 상태값
 */

export type ProductStatus = 'SELLING' | 'RESERVED' | 'SOLD'

export const STATUS_LABEL: Record<ProductStatus, string> = {
  SELLING: '판매중',
  RESERVED: '예약중',
  SOLD: '판매완료',
}

export const CATEGORIES = [
  '디지털기기',
  '생활가전',
  '가구/인테리어',
  '생활/주방',
  '유아동',
  '의류',
  '스포츠/레저',
  '도서',
  '게임/취미',
  '뷰티/미용',
  '반려동물용품',
  '기타',
] as const

export type Category = (typeof CATEGORIES)[number]

export interface Seller {
  id: string
  nickname: string
  avatarUrl: string
  mannerScore: number
}

export interface Product {
  id: string
  title: string
  description: string
  price: number
  status: ProductStatus
  category: Category
  thumbnailUrl: string
  images: string[]
  location: string
  timeAgo: string
  seller: Seller
  viewCount: number
  chatCount: number
  likeCount: number
}

export const products: Product[] = [
  {
    id: '1',
    title: 'Apple Watch Ultra 2 거의 새것',
    description:
      '작년 12월에 구매한 애플워치 울트라 2입니다. 사용감 거의 없고 박스, 충전기 모두 포함입니다. 직거래 선호하며 Duluth H Mart 근처에서 만나요.',
    price: 580,
    status: 'SELLING',
    category: '디지털기기',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1434493789847-2f02dc6ca35d?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1434493789847-2f02dc6ca35d?w=800&h=800&fit=crop',
      'https://images.unsplash.com/photo-1546868871-7041f2a55e12?w=800&h=800&fit=crop',
    ],
    location: 'Duluth',
    timeAgo: '3분 전',
    seller: {
      id: 'u1',
      nickname: '김민수',
      avatarUrl: 'https://i.pravatar.cc/150?u=1',
      mannerScore: 4.8,
    },
    viewCount: 45,
    chatCount: 3,
    likeCount: 12,
  },
  {
    id: '2',
    title: 'Samsung Galaxy Buds3 Pro 미개봉',
    description:
      '회사에서 받은 선물인데 이미 AirPods이 있어서 판매합니다. 미개봉 새상품이고 영수증도 있습니다.',
    price: 180,
    status: 'RESERVED',
    category: '디지털기기',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=800&h=800&fit=crop',
    ],
    location: 'Suwanee',
    timeAgo: '15분 전',
    seller: {
      id: 'u2',
      nickname: '이지영',
      avatarUrl: 'https://i.pravatar.cc/150?u=2',
      mannerScore: 5.0,
    },
    viewCount: 89,
    chatCount: 7,
    likeCount: 24,
  },
  {
    id: '3',
    title: 'Dyson Airwrap Complete',
    description:
      '1년 정도 사용했습니다. 구성품 모두 있고 상태 양호합니다. 이사 가면서 정리합니다.',
    price: 320,
    status: 'SOLD',
    category: '뷰티/미용',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1522338242042-2d1c2665861a?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1522338242042-2d1c2665861a?w=800&h=800&fit=crop',
    ],
    location: 'Johns Creek',
    timeAgo: '1시간 전',
    seller: {
      id: 'u3',
      nickname: '박서연',
      avatarUrl: 'https://i.pravatar.cc/150?u=3',
      mannerScore: 4.9,
    },
    viewCount: 156,
    chatCount: 12,
    likeCount: 45,
  },
  {
    id: '4',
    title: 'iPhone 15 Pro Max 256GB Black',
    description:
      '사용한 지 3개월 됐습니다. 케이스 끼고 사용해서 기스 없습니다. 배터리 성능 99%. 박스, 충전기 포함.',
    price: 950,
    status: 'SELLING',
    category: '디지털기기',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1695048133142-1a20484d2569?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1695048133142-1a20484d2569?w=800&h=800&fit=crop',
      'https://images.unsplash.com/photo-1510557880182-3d4d3cba35a5?w=800&h=800&fit=crop',
    ],
    location: 'Duluth',
    timeAgo: '2시간 전',
    seller: {
      id: 'u4',
      nickname: '최준혁',
      avatarUrl: 'https://i.pravatar.cc/150?u=4',
      mannerScore: 4.7,
    },
    viewCount: 234,
    chatCount: 8,
    likeCount: 67,
  },
  {
    id: '5',
    title: 'MacBook Pro 14" M3 Pro Silver',
    description:
      '2024년 1월 구매. 업무용으로 사용했고 집에서만 사용해서 상태 최상입니다. AppleCare+ 2025년 12월까지.',
    price: 1800,
    status: 'SELLING',
    category: '디지털기기',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=800&h=800&fit=crop',
      'https://images.unsplash.com/photo-1541807084-5c52b6b3adef?w=800&h=800&fit=crop',
    ],
    location: 'Alpharetta',
    timeAgo: '3시간 전',
    seller: {
      id: 'u5',
      nickname: '정다은',
      avatarUrl: 'https://i.pravatar.cc/150?u=5',
      mannerScore: 5.0,
    },
    viewCount: 312,
    chatCount: 15,
    likeCount: 89,
  },
  {
    id: '6',
    title: 'Nintendo Switch OLED + 게임 5개',
    description:
      '스위치 OLED 화이트 모델입니다. Zelda, Mario Kart, Animal Crossing 등 게임 5개 포함. Pro Controller도 같이 드립니다.',
    price: 250,
    status: 'SELLING',
    category: '게임/취미',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1578303512597-81e6cc155b3e?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1578303512597-81e6cc155b3e?w=800&h=800&fit=crop',
    ],
    location: 'Duluth',
    timeAgo: '5시간 전',
    seller: {
      id: 'u6',
      nickname: '한소희',
      avatarUrl: 'https://i.pravatar.cc/150?u=6',
      mannerScore: 4.6,
    },
    viewCount: 98,
    chatCount: 4,
    likeCount: 23,
  },
  {
    id: '7',
    title: 'LG StanbyME 27인치',
    description:
      '이동식 TV로 정말 편합니다. 구매한 지 6개월 됐고 상태 좋습니다. 직거래만 가능합니다.',
    price: 400,
    status: 'RESERVED',
    category: '생활가전',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1593359677879-a4bb92f829d1?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1593359677879-a4bb92f829d1?w=800&h=800&fit=crop',
    ],
    location: 'Suwanee',
    timeAgo: '6시간 전',
    seller: {
      id: 'u7',
      nickname: '윤재민',
      avatarUrl: 'https://i.pravatar.cc/150?u=7',
      mannerScore: 4.9,
    },
    viewCount: 187,
    chatCount: 9,
    likeCount: 56,
  },
  {
    id: '8',
    title: 'Sony WH-1000XM5 Headphones',
    description:
      '노이즈캔슬링 최고입니다. 8개월 사용했고 케이스, 충전케이블 포함입니다.',
    price: 200,
    status: 'SELLING',
    category: '디지털기기',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1618366712010-f4ae9c647dcb?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1618366712010-f4ae9c647dcb?w=800&h=800&fit=crop',
    ],
    location: 'Buford',
    timeAgo: '8시간 전',
    seller: {
      id: 'u8',
      nickname: '송민아',
      avatarUrl: 'https://i.pravatar.cc/150?u=8',
      mannerScore: 4.8,
    },
    viewCount: 76,
    chatCount: 2,
    likeCount: 18,
  },
  {
    id: '9',
    title: 'BALMUDA Toaster White',
    description:
      '발뮤다 토스터 화이트입니다. 토스트 정말 맛있게 구워집니다. 박스 있습니다.',
    price: 120,
    status: 'SELLING',
    category: '생활/주방',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1585237017125-24baf8d7406f?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1585237017125-24baf8d7406f?w=800&h=800&fit=crop',
    ],
    location: 'Duluth',
    timeAgo: '1일 전',
    seller: {
      id: 'u9',
      nickname: '임수진',
      avatarUrl: 'https://i.pravatar.cc/150?u=9',
      mannerScore: 4.5,
    },
    viewCount: 54,
    chatCount: 1,
    likeCount: 9,
  },
  {
    id: '10',
    title: '캠핑 텐트 4인용 + 타프 세트',
    description:
      '코베아 텐트와 타프 세트입니다. 5번 정도 사용했고 상태 좋습니다. 캠핑 그만두면서 정리합니다.',
    price: 150,
    status: 'SELLING',
    category: '스포츠/레저',
    thumbnailUrl:
      'https://images.unsplash.com/photo-1504280390367-361c6d9f38f4?w=400&h=400&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1504280390367-361c6d9f38f4?w=800&h=800&fit=crop',
      'https://images.unsplash.com/photo-1478827536114-da961b7f86d2?w=800&h=800&fit=crop',
    ],
    location: 'Lawrenceville',
    timeAgo: '1일 전',
    seller: {
      id: 'u10',
      nickname: '강동원',
      avatarUrl: 'https://i.pravatar.cc/150?u=10',
      mannerScore: 4.7,
    },
    viewCount: 123,
    chatCount: 5,
    likeCount: 34,
  },
]

export const getProductById = (id: string): Product | undefined => {
  return products.find((p) => p.id === id)
}
